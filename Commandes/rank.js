    ...
    db.query(`SELECT * FROM xp WHERE guildID ='${message.guildId}' AND user = '${user.id}'`, async (err, req) => {
      ...
      db.query(`SELECT * FROM xp WHERE guildID = '${message.guildId}'`, async (err, all) => {
        ...
        let Card = await new Canvas.Card()
          .setBackground("https://cdn.discordapp.com/attachments/1126855731946213427/1137007432242045080/Copie_de_Copie_de_Copie_de_Copie_de_Copie_de_Copie_de_Copie_de_Copie_de_Copie_de_Copie_de_Copie_de_Copie_de_ETHEREAL.jpg")
          .setBot(bot)
          .setColorFont('#ffffff')
          .setRank(rank + 1)
          .setUser(user)
